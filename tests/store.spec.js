...
describe("API store", () => {
  [true, false].forEach((avoidNPlusOneRequests) => {
    const title = avoidNPlusOneRequests
      ? "avoiding n+1 queries"
      : "not avoiding n+1 queries";
    describe(title, () => {
      beforeAll(() => {
      ...
      it("can serialize Resource object", async () => {
        ...
        expect(loadedObject.toJSON()).toEqual(
          '{"this":"is","the":"root","_meta":{"self":"","loading":false,"reloading":false,"load":"{}"}}'
        );
      });
      ...
      it("imports embedded single entity", async () => {
        ...
        expect(vm.$store.state.api).toMatchObject({
          "/camps/1": { _meta: { self: "/camps/1", loading: true } },
        });
        ...
        expect(vm.api.get("/camps/1").campType()._meta.self).toEqual(
          "/campTypes/20"
        );
      ...
      it("imports reference to single entity", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .reply(200, referenceToSingleEntity.serverResponse);
        ...
        await letNetworkRequestFinish();
        expect(vm.$store.state.api).toMatchObject(
          referenceToSingleEntity.storeState
        );
        expect(vm.api.get("/camps/1")._meta.self).toEqual("/camps/1");
      });
      ...
      it("imports embedded collection", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .reply(200, embeddedCollection.serverResponse);
        ...
        await letNetworkRequestFinish();
        expect(vm.$store.state.api).toMatchObject(
          embeddedCollection.storeState
        );
        expect(vm.api.get("/camps/1")._meta.self).toEqual("/camps/1");
        expect(vm.api.get("/camps/1").periods().items[0]._meta.self).toEqual(
          "/periods/104"
        );
        expect(vm.api.get("/camps/1").periods().items[1]._meta.self).toEqual(
          "/periods/128"
        );
        expect(vm.api.get("/periods/104")._meta.self).toEqual("/periods/104");
        expect(vm.api.get("/periods/104").camp()._meta.self).toEqual(
          "/camps/1"
        );
        expect(vm.api.get("/periods/128")._meta.self).toEqual("/periods/128");
        expect(vm.api.get("/periods/128").camp()._meta.self).toEqual(
          "/camps/1"
        );
      });
      ...
      it("imports embedded collection with standalone link", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .reply(200, embeddedCollectionStandaloneLink.serverResponse);
        ...
        await letNetworkRequestFinish();
        expect(vm.$store.state.api).toMatchObject(
          embeddedCollectionStandaloneLink.storeState
        );
        expect(vm.api.get("/camps/1")._meta.self).toEqual("/camps/1");
        expect(vm.api.get("/camps/1").periods().items[0]._meta.self).toEqual(
          "/periods/104"
      ...
      it("imports embedded collection with link array", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .reply(200, embeddedCollectionLinkArray.serverResponse);
        ...
        await letNetworkRequestFinish();
        expect(vm.$store.state.api).toMatchObject(
          embeddedCollectionLinkArray.storeState
        );
        expect(vm.api.get("/camps/1")._meta.self).toEqual("/camps/1");
        expect(vm.api.get("/camps/1").periods().items[0]._meta.self).toEqual(
          "/periods/104"
      ...
      it("imports linked single entity", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .reply(200, linkedSingleEntity.serverResponse);
        const mainLeader = {
          serverResponse: {
            id: 83,
            name: "Smiley",
            _links: { self: { href: "/users/83" } },
          },
          storeState: { id: 83, name: "Smiley", _meta: { self: "/users/83" } },
        };
        axiosMock
          .onGet("http://localhost/users/83")
          .reply(200, mainLeader.serverResponse);
        ...
        await letNetworkRequestFinish();
        expect(vm.$store.state.api).toMatchObject(
          linkedSingleEntity.storeState
        );
        expect(vm.api.get("/camps/1")._meta.self).toEqual("/camps/1");
        // expect(vm.api.get('/camps/1').main_leader()._meta.self).toEqual('/users/83')
        expect(vm.api.get("/camps/1").main_leader()._meta.loading).toEqual(
          true
        );
        await letNetworkRequestFinish();
        expect(vm.api.get("/camps/1").main_leader()._meta).toMatchObject({
          self: "/users/83",
        });
      ...
      it("imports linked entity array", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .reply(200, linkedEntityArray.serverResponse);

        const activity1 = {
          serverResponse: {
            id: 1,
            title: "LS Volleyball",
            _links: { self: { href: "/activities/1" } },
          },
          storeState: {
            id: 1,
            title: "LS Volleyball",
        ...
        axiosMock
          .onGet("http://localhost/activities/1")
          .reply(200, activity1.serverResponse);

        const activity2 = {
          serverResponse: {
            id: 2,
            title: "LA Blachen",
            _links: { self: { href: "/activities/2" } },
          },
          storeState: {
            id: 2,
            title: "LA Blachen",
        ...
        axiosMock
          .onGet("http://localhost/activities/2")
          .reply(200, activity2.serverResponse);
        ...
        expect(vm.$store.state.api["/activities/1"]).toMatchObject(
          activity1.storeState
        );
        expect(vm.$store.state.api["/activities/2"]).toMatchObject(
          activity2.storeState
        );
        expect(vm.api.get("/camps/1").activities().items.length).toEqual(2);
        expect(vm.api.get("/camps/1").activities().items[0].title).toEqual(
          "LS Volleyball"
        );
        expect(vm.api.get("/camps/1").activities().items[1].title).toEqual(
          "LA Blachen"
        );
      });
      ...
      it("imports paginatable collection", async () => {
              ...
              items: [
                {
                  id: 1234,
                  title: "LS Volleyball",
                  _links: { self: { href: "/activities/1234" } },
                },
                {
                  id: 1236,
                  title: "LA Blachen",
                  _links: { self: { href: "/activities/1236" } },
                },
              ],
            },
            _links: {
              self: { href: "/camps/1/activities" },
              first: { href: "/camps/1/activities" },
            },
            _page: 0,
            _per_page: -1,
            _total: 2,
        ...
        axiosMock
          .onGet("http://localhost/camps/1/activities")
          .reply(200, activities.serverResponse);
        ...
        await letNetworkRequestFinish();
        expect(vm.$store.state.api["/camps/1/activities"]).toMatchObject(
          activities.storeState
        );
        expect(vm.api.get("/camps/1").activities().items.length).toEqual(2);
        expect(vm.api.get("/camps/1").activities().items[0]._meta.self).toEqual(
          "/activities/1234"
        );
        expect(vm.api.get("/camps/1").activities().items[1]._meta.self).toEqual(
          "/activities/1236"
        );
      });
      ...
      it("imports paginatable collection with multiple pages", async () => {
        // given
        axiosMock
          .onGet("http://localhost/camps/1/activities?page=0&page_size=2")
          .reply(200, collectionFirstPage.serverResponse);
        axiosMock
          .onGet("http://localhost/camps/1/activities?page=1&page_size=2")
          .reply(200, collectionPage1.serverResponse);
        ...
        expect(
          vm.api.get("/camps/1/activities?page_size=2&page=0").items.length
        ).toEqual(2);
        ...
        expect(vm.$store.state.api).toMatchObject({
          ...collectionFirstPage.storeState,
          ...collectionPage1.storeState,
        });
        expect(
          vm.api.get("/camps/1/activities?page_size=2&page=0").items.length
        ).toEqual(2);
        expect(
          vm.api.get("/camps/1/activities?page_size=2&page=1").items.length
        ).toEqual(1);
      });
      ...
      it("reloads a URI from the store", async () => {
        ...
        axiosMock
          .onGet("http://localhost/campTypes/20")
          .reply(200, campType.serverResponse);
        vm.api.get("/camps/1");
        await letNetworkRequestFinish();
      ...
      it("reloads an object from the store", async () => {
        ...
        axiosMock
          .onGet("http://localhost/campTypes/20")
          .reply(200, campTypeData.serverResponse);
      ...
      it("reloads an embedded collection from the store by reloading the superordinate object", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .replyOnce(200, campData.serverResponse);
        axiosMock
          .onGet("http://localhost/camps/1")
          .replyOnce(200, campData.serverResponse2);
        ...
        await letNetworkRequestFinish();
        expect(
          vm.$store.state.api["/camps/1#activityTypes"].items
        ).toMatchObject(campData.storeState);
      });
      ...
      it("reloads a linked collection without standalone link from the store by reloading the superordinate object", async () => {
        ...
        const campServerResponse2 = {
          ...
          _links: {
            ...
            activityTypes: [{ href: "/activityTypes/125" }],
          },
        };
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .replyOnce(200, campServerResponse1);
        axiosMock
          .onGet("http://localhost/camps/1")
          .replyOnce(200, campServerResponse2);
        vm.api.get("/camps/1").activityTypes();
        await letNetworkRequestFinish();
        const embeddedCollection = vm.api.get("/camps/1").activityTypes();
        ...
        expect(
          vm.$store.state.api["/camps/1#activityTypes"].items
        ).toMatchObject(activityTypeStoreState);
      });
      ...
      it("loads the contents of an embedded collection depending on the avoidNPlusOneQueries flag", async () => {
        ...
        if (avoidNPlusOneRequests) {
          axiosMock
            .onGet("http://localhost/books/555")
            .replyOnce(200, bookResponse);
        } else {
          axiosMock
            .onGet("http://localhost/chapters/1028")
            .replyOnce(200, chapter1Response);
          axiosMock
            .onGet("http://localhost/chapters/1031")
            .replyOnce(200, chapter2Response);
          axiosMock
      ...
      it("deletes an URI from the store and reloads all entities referencing it", async () => {
        // given
        axiosMock
          .onGet("http://localhost/groups/99")
          .replyOnce(200, multipleReferencesToUser);
      ...
      it("breaks circular dependencies when deleting an entity in the reference circle", async () => {
        ...
        axiosMock
          .onGet("http://localhost/periods/1")
          .replyOnce(200, circularReference.serverResponse);
      ...
      it("breaks circular dependencies when deleting an entity outside the reference circle", async () => {
        // given
        axiosMock
          .onGet("http://localhost/camps/3")
          .replyOnce(200, circularReference.campServerResponse);
        axiosMock
          .onGet("http://localhost/periods/1")
          .replyOnce(200, circularReference.serverResponse);
      ...
      it("automatically filters deleting items from standalone collection", async () => {
        ...
        expect(
          standaloneCollection.items.some((item) => item._meta.deleting)
        ).toBeFalsy();
        expect(standaloneCollection.items.length).toEqual(1);
        expect(
          standaloneCollection.allItems.some((item) => item._meta.deleting)
        ).toBeTruthy();
      ...
      it("automatically filters deleting items from embedded collection", async () => {
        ...
        expect(
          embCollection.items.some((item) => item._meta.deleting)
        ).toBeFalsy();
        expect(embCollection.items.length).toEqual(1);
        expect(
          embCollection.allItems.some((item) => item._meta.deleting)
        ).toBeTruthy();
      ...
      it("still returns old instance from store while patch is in progress", async () => {
        ...
        // then
        expect(vm.$store.state.api["/camps/1"]).toMatchObject({
          id: 1,
          _meta: { self: "/camps/1" },
        });
        expect(vm.$store.state.api["/campTypes/20"]).toMatchObject({
      ...
      it("posts entity and stores the response into the store", async () => {
        ...
        expect(axiosPostSpy).toHaveBeenCalledWith(
          expect.objectContaining({ data: JSON.stringify({ some: "thing" }) })
        ); // verify correct data has been sent to axios
        expect(await load).toMatchObject({
          id: 1,
          _meta: { self: "/camps/1" },
        });
      ...
      it("gets the href of a linked entity without fetching the entity itself", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .replyOnce(200, linkedSingleEntity.serverResponse);
      ...
      it("gets the href of a templated linked entity without fetching the entity itself", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .replyOnce(200, templatedLink.linkingServerResponse);
        axiosMock.onGet("http://localhost/users/83").networkError();
        ...
        expect(vm.$store.state.api).toMatchObject(
          templatedLink.storeStateBeforeLinkedLoaded
        );
      });
      ...
      it("imports templated link to single entity when linking entity is still loading", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1/users/83")
          .reply(200, templatedLink.linkedServerResponse);
        ...
        await letNetworkRequestFinish();
        expect(vm.$store.state.api).toMatchObject(
          templatedLink.storeStateAfterLinkedLoaded
        );
        expect(await load).toMatchObject({
          id: 83,
          name: "Pflock",
      ...
      it("imports templated link to single entity when linking entity is already loaded", async () => {
        ...
        expect(vm.$store.state.api).not.toMatchObject(
          templatedLink.storeStateAfterLinkedLoaded
        );
        await letNetworkRequestFinish();
        expect(vm.$store.state.api).toMatchObject(
      ...
      it("returns error when `get` encounters network error", async () => {
        ...
        await expect(load).rejects.toThrow(
          'Error trying to fetch "/camps/1": Network Error'
        );
      });
      ...
      it("returns error when `get` encounters network timeout", async () => {
        ...
        // then
        await expect(load).rejects.toThrow(
          'Error trying to fetch "/camps/1": timeout of 0ms exceeded'
        );
      });
      ...
      it("returns error when `get` encounters 404 Not Found", async () => {
        ...
        // then
        await expect(load).rejects.toThrow(
          'Could not fetch "/camps/1" (status 404): Request failed with status code 404'
        );
      ...
      it("returns error when `get` encounters 403 Forbidden", async () => {
        ...
        // then
        await expect(load).rejects.toThrow(
          'No permission to fetch "/camps/1" (status 403): Request failed with status code 403'
        );
      });
      ...
      it("returns error when `reload` encounters 404 Not Found", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .replyOnce(200, { id: 1, _links: { self: { href: "/camps/1" } } });
        axiosMock.onGet("http://localhost/camps/1").replyOnce(404);
        ...
        // then
        await expect(load).rejects.toThrow(
          'Could not reload "/camps/1" (status 404): Request failed with status code 404'
        );
        expect(vm.$store.state.api["/camps/1"]).toBeUndefined();
      });
      ...
      it("returns error when `patch` encounters network error", async () => {
        ...
        // then
        await expect(load).rejects.toThrow(
          'Error trying to patch "/camps/1": Network Error'
        );
      });
      ...
      it("returns error when `patch` encounters network timeout", async () => {
        ...
        // then
        await expect(load).rejects.toThrow(
          'Error trying to patch "/camps/1": timeout of 0ms exceeded'
        );
      });
      ...
      it("returns error when `patch` encounters 404 Not Found", async () => {
        ...
        // then
        await expect(load).rejects.toThrow(
          'Could not patch "/camps/1" (status 404): Request failed with status code 404'
        );
      ...
      it("returns error when `patch` encounters 403 Forbidden", async () => {
        ...
        // then
        await expect(load).rejects.toThrow(
          'No permission to patch "/camps/1" (status 403): Request failed with status code 403'
        );
      });

      it("returns error when `patch` encounters 422 Unprocessable Entity (Validation error)", async () => {
        // given
        axiosMock.onPatch("http://localhost/camps/1").replyOnce(
          422,
          {
            validation_messages: {
              title: {
        ...
        // then
        await expect(load).rejects.toThrow(
          'Error trying to patch "/camps/1" (status 422): Request failed with status code 422'
        );
      });
      ...
      it("can handle object property", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .reply(200, objectProperty.serverResponse);
      ...
      it("can handle array property", async () => {
        ...
        axiosMock
          .onGet("http://localhost/camps/1")
          .reply(200, arrayProperty.serverResponse);
      ...
      it("throws error when accessing non-existing property like a relation", async () => {
        ...
        // then (loaded)
        await expect(loadingObject._meta.load).rejects.toThrow(
          "Property 'nonexistingProperty' on resource '' was used like a relation, but no relation with this name was returned by the API (actual return value: undefined)"
        );
      });
      ...
      it("throws error when accessing primitive property like a relation", async () => {
        ...
        // then (loaded)
        await expect(loadingObject._meta.load).rejects.toThrow(
          "Property 'the' on resource '' was used like a relation, but no relation with this name was returned by the API (actual return value: \"root\")"
        );
      });
      ...
      it("exposes absolute Url on loaded resources", async () => {
        ...
        expect(camp).toMatchObject({
          id: 1,
          _meta: { self: "/camps/1", selfUrl: "http://localhost/camps/1" },
        });
      });
